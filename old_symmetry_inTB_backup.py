...
##### this one is new but I took out because its buggy
# doesn't work for 1.08_0fold_no18

Hint: 1) Maybe try with exact boundary, not the residue 200 atoms
2) check how it was working for zxact?

  
    def check_Hsymm(self, H, name, k_label, rtol=0.1, ftol=0):
        """
        ...
        H_prime = sp.lil_matrix(sp.linalg.inv(self.Cmat[k_label][name]) @ org @ self.Cmat[k_label][name])
        
        nonZ = org.nonzero()
        nonZ_tot = nonZ[0].shape[0]
        
        
        print('checking H invariance under ', name, ' at ', k_label)
        pbar = tqdm(total=nonZ_tot, unit=' check', desc='inv('+name+') '+'H('+k_label+') '+name) # Initialise
        case = True
        f_buffer = 0
        for zz in range(nonZ_tot):
            i = nonZ[0][zz]
            j = nonZ[1][zz]
            if not np.isclose( org[i, j], H_prime[i, j], rtol=rtol, atol=0):
                f_buffer += 1
                print(org[i, j], H_prime[i, j])
                if f_buffer > ftol:
                    print(org[i, j], H_prime[i, j])
                    case = False
                    break
        ...
        if case: 
            print('H is invariance under ',name, ' at ', k_label,'. Buffer: ', f_buffer)
        else:
            print('Buffer exceeded: ', f_buffer, ' H is **not** invariance under '+name, ' at ', k_label)
            

## this one aslo is useful but buggy

    def plotter_waves(self, name, flat_id1, type_1, flat_id2, type_2):
        """
            Visualize wavevectors. Useful for understanding symmetry
        """
        
        flat_range= self.tb.n_flat
        #wave_info = np.zeros((flat_range, self.conf.tot_number, 6), self.tb.dtypeR)
        ...
        #for ii in range(flat_range):
            ...
            #wave_info[ii, :, 3] = np.absolute(self.new_bases[ii, :])
            #wave_info[ii, :, 4] = np.angle(self.new_bases[ii, :], deg=True)
            #wave_info[ii, :, 5] = self.conf.atomsAllinfo[:, 2] # type
        
        lis1 = self.conf.atomsAllinfo[:, 2] == type_1
        lis2 = self.conf.atomsAllinfo[:, 2] == type_2
        #lis2 = self.conf.atomsAllinfo[self.new_orders[name], 2] == type_2
        
        
        fig, axs = plt.subplots(1,2,figsize=(7, 10))
        cm = plt.cm.get_cmap('RdYlBu')
        
        sc0 = axs[0].scatter(self.conf.coords[lis1, 0], self.conf.coords[lis1, 1], 
                            c=np.angle(self.new_bases[flat_id1, lis1], deg=True),  cmap=cm, 
                            s= 4000*np.absolute(self.new_bases[flat_id1, lis1]), alpha= 0.25 )
        
        #k_label = 'W'
        #trans_base = sp.lil_matrix(self.new_bases[flat_id2]) @ self.Cmat[k_label][name] 
        #xx = sp.lil_matrix(self.conf.coords[:, 0]) @ self.Cmat[k_label][name] 
        #yy = sp.lil_matrix(self.conf.coords[:, 1]) @ self.Cmat[k_label][name] 
        #tt = sp.lil_matrix(self.conf.atomsAllinfo[:, 2]) @ self.Cmat[k_label][name] 
        
        ##trans_base = trans_base.todense()
        ##print(trans_base.shape)
        #trans_base = np.array(trans_base.todense())[0]
        #xx = np.array(xx.todense())[0]
        #yy = np.array(yy.todense())[0]
        #tt = np.array(tt.todense())[0]
        #print(trans_base.shape)
        
        sc1 = axs[1].scatter(self.conf.coords[lis2, 0], self.conf.coords[lis2, 1], 
                            c=np.angle(self.new_bases[flat_id2, lis2], deg=True),  cmap=cm, 
                            s= 4000*np.absolute(self.new_bases[flat_id2, lis2]), alpha= 0.25 )
        
        #sc1 = axs[1].scatter((self.conf.coords[self.new_orders[name], 0])[lis2], (self.conf.coords[self.new_orders[name], 1])[lis2], 
                            #c=(np.angle(self.new_bases[flat_id2][self.new_orders[name]], deg=True))[lis2],  cmap=cm, 
                            #s= 4000*(np.absolute(self.new_bases[flat_id2][self.new_orders[name]]))[lis2], alpha= 0.25 )
        
        
        fig.colorbar(sc0,orientation='vertical', shrink=0.5)
        fig.colorbar(sc1,orientation='vertical', shrink=0.5)
        
        #plt.colorbar(sc)
        axs[0].set_title('Vector {0} orbital type {1}'.format(flat_id1, type_1))
        axs[1].set_title(name+' Vector {0} orbital type {1}'.format(flat_id2, type_2))
